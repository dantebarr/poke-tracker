...
Task capture requires a title, a due date, a label and a size — all four are NOT NULL in the schema. Capture is single-step: every field is reachable without navigating away, Save is disabled until it validates, and there are no wizards.